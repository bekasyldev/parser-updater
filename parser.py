...
class MarketplaceParser:
    def __init__(self):
        ...
        self.options.add_argument(
            f'--proxy-server=http://{PROXY_HOST}:{PROXY_PORT}@{PROXY_HOST}:{PROXY_PORT}')

        # Basic settings
        # Using newer headless mode
        ...
        try:
            ...
            logging.info(
                f"Chrome driver initialized successfully with binary: {CHROME_BINARY}")
    ...
    def parse_kaspi(self, url):
        try:
            ...
            if "robots" in self.driver.page_source.lower() or len(
            ...
            try:
                out_of_stock = self.driver.find_elements(
                    By.CSS_SELECTOR, '.sold-out-text')
                if out_of_stock and any(elem.is_displayed()
                                        for elem in out_of_stock):
            ...
            try:
                ...
                if match:
                    ...
                    try:
                        reviews_elem = self.driver.find_element(
                            By.CSS_SELECTOR, 'div.rating__counter')
                        data['total_reviews'] = int(
                            ''.join(filter(str.isdigit, reviews_elem.text)))

                        rating_elem = self.driver.find_element(
                            By.CSS_SELECTOR, 'div.rating__digits')
                ...
                logging.info(
                    f"Reviews data: total={
                        data['total_reviews']}, rating={
                        data['rating']}")
    ...
    def parse_alibaba(self, url):
        try:
            ...
            # Check availability
            try:
                unavailable = self.driver.find_element(
                    By.CLASS_NAME, "product-unsafe")
            ...
            try:
                price_elem = self.driver.find_element(
                    By.CSS_SELECTOR, "div.price-list .price")
            ...
            try:
                reviews = self.driver.find_element(
                    By.CSS_SELECTOR, "div.verified-reviews")
            ...
            try:
                delivery = self.driver.find_element(
                    By.CSS_SELECTOR, "div.detail-next-progress-line-text")
    ...
    def parse_wildberries(self, url):
        try:
            ...
            try:
                out_of_stock = self.driver.find_elements(
                    By.CSS_SELECTOR, "p.sold-out-product")
                if out_of_stock:
                    return {'product_url': url, 'is_available': False}
            except BaseException:
                pass
            ...
            try:
                ...
                for selector in price_selectors:
                    try:
                        ...
                        if elements:
                            for elem in elements:
                                if elem.text and any(c.isdigit()
                                                     for c in elem.text):
            ...
            # Get reviews and rating
            try:
                reviews_count_elem = self.driver.find_element(
                    By.CSS_SELECTOR, "span.product-review__count-review")
                if reviews_count_elem:
                    print(reviews_count_elem.text)
                    data['reviews'] = ''.join(re.findall(
                        r'\d+', reviews_count_elem.text))
                else:
                    data['reviews'] = '0'

                rating_elem = self.driver.find_element(
                    By.CSS_SELECTOR, "span.product-review__rating")
                if rating_elem:
    ...
    def parse_ozon(self, url):
        try:
            ...
            try:
                ...
                price_elem = self.driver.find_element(
                    By.CSS_SELECTOR, "span.l8t_27.tl8_27.l2u_27")
                if price_elem:
            ...
            # Get reviews and rating
            try:
                reviews_elem = self.driver.find_elements(
                    By.CSS_SELECTOR, "div.ga121-a2.tsBodyControl500Medium")
                # reviews_elem.text is like "4.8 • 14 006 reviews" format
                # "rating • reviews_count reviews"
                data['reviews'] = ''.join(
                    re.findall(
                        r'\d+',
                        reviews_elem[0].text.split('•')[1]))
